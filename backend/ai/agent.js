// backend/ai/agent.js
// JD generation using Azure OpenAI

import { HAY_SYSTEM_PROMPT } from "./hayKnowledge.js";
import { azurePrompt, AZURE_MODEL_LABEL } from "./azureClient.js";

async function callLLM(userPrompt) {
  return azurePrompt(HAY_SYSTEM_PROMPT, userPrompt, { temperature: 0.3 });
}
...
export async function generateJD(wizardData) {
  ...
  const p_jobPurpose =
    ...
    "LOB: " + ctx.lob + " | Business: " + ctx.business + " | Function: " + ctx.function + "\n" +
    "Department: " + ctx.department + " | Designation: " + ctx.designation + " | Band: " + ctx.band + "\n" +
    ctx.bcSummary +
    "Raw input from HR: " + (wizardData.jobPurpose || "Not provided") + "\n" +
    "Dimensions: " + dimsText + "\n\n" +
    "Match band-appropriate verbs. Remember JB 1 is highest and JB 10 is lowest. This role is " + ctx.band + ".\n" +
    "Quantify from dimensions. Third person present tense.\n" +
    "Return ONLY the paragraph.";

  const p_orgContext =
    "Write the Organisation Context paragraph for " + (ctx.business || "the business") + ".\n" +
    "Cover entity name and lineage, AUM or scale, market rank, distribution footprint, employee strength, product breadth, and 2-3 competitive strengths.\n" +
    "Raw HR input: " + (wizardData.orgContext || "Not provided") + "\n" +
    "Reference dimensions where available: " + dimsText + "\n\n" +
    "Return ONLY the paragraph. 200-300 words.";

  const p_jobContext =
    "Write 5-8 bullet points describing the Job Context for this " + ctx.band + " " + ctx.designation +
    " role in " + ctx.department + ".\n" +
    "Each bullet should describe a scope or oversight area.\n" +
    "Raw HR input: " + (wizardData.jobContext || "Not provided") + "\n\n" +
    "Return as a numbered list. Third person present tense. Start with band-appropriate verbs.";

  const p_challenges =
    "Convert this into 8-12 formal Hay-grade Key Challenge statements.\n" +
    "Context: " + ctx.lob + " | " + ctx.department + " | " + ctx.band + " | " + ctx.designation + "\n" +
    "Raw challenges from HR: " + JSON.stringify(wizardData.challenges || []) + "\n\n" +
    "Each challenge MUST:\n" +
    "- Start with a complexity signal such as Navigating, Balancing, Interpreting, Managing competing priorities, Ensuring, To deploy, or To ensure\n" +
    "- Reference specific regulator or market reality where applicable\n" +
    "- Quantify or qualify where possible\n\n" +
    "Return as a numbered list, one challenge per line.";

  const p_accountabilities =
    "Transform these raw accountabilities into a structured list matching the ABSLAMC JD template.\n" +
    "Context: " + ctx.lob + " | " + ctx.department + " | " + ctx.band + "\n" +
    "Raw input: " + rawAcc + "\n\n" +
    "For each accountability, produce:\n" +
    "- A short accountability heading of 2-6 words\n" +
    "- 3-6 supporting action bullets, each starting with a band-appropriate verb\n\n" +
  ...
  const reporteeSummary = hasNoReportees
    ...
    : effectiveReportees.length === 0
      ...
      : effectiveReportees
        .map((r) => `- ${r.count} x ${r.band || "unspecified band"} in ${r.department || "unspecified dept"}`)
        .join("\n");

  const p_directReports = hasNoReportees
    ? null
    : "Write formal Job Purpose descriptions for these direct reports of a " + ctx.band + " " + ctx.designation + ".\n" +
    "Reportee groups:\n" + reporteeSummary + "\n\n" +
  ...
  const [jobPurpose, orgContext, jobContext, challenges, accountabilitiesRaw] = results;
